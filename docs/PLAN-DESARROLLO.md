...
- ✅ Dashboard de cliente con navegación por 5 pestañas (Resumen, Mis equipos, Club Blufil, Referidos, Facturas) — ver sección propia más abajo.
...
- **Aviso de nueva solicitud de servicio**: nueva Edge Function `notificar-solicitud` (dispara desde `solicitarMantenimiento` en `portal/src/app/dashboard/actions.ts`, mismo patrón "fire and forget" que `facturar-visita`). Envía un correo vía **Resend** con los datos de la solicitud. **Hoy solo llega a un correo admin fijo** (`saidtorrente@gmail.com`, constante en el código) — decisión explícita del usuario porque hoy `tecnicos` no tiene columna de teléfono y tiene 0 filas en producción. **Pendiente activar**: sumar a `tecnicos` disponibles/certificados con correo en la función `destinatarios()` de la Edge Function cuando haya técnicos reales cargados — el comentario `// TODO` ya está en el código para no perderlo. Requiere que el usuario cree la cuenta de Resend, verifique el dominio `blufil.com` y pegue la API key en Supabase Vault (`resend_api_key`) — sin eso, la función responde con un warning y no bloquea la solicitud.
