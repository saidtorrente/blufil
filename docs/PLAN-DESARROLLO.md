...
- ✅ Fase 3 (portal de cliente, MVP): app Next.js en [`portal/`](../portal/), **en vivo en https://portal.blufil.com** (Hostinger, subdominio Node.js). Login **por cédula + contraseña** (no por correo/OTP como en el primer intento): contraseña inicial = número de cédula, cuenta bloqueada hasta confirmar el correo (Supabase Auth), y el cliente puede cambiar su contraseña desde el portal (`/dashboard/cambiar-clave`). Dos Edge Functions en Supabase: `iniciar-sesion` (resuelve cédula → correo con la service role, sin exponerla nunca al portal) y `provisionar-cliente` (crea la cuenta de Auth + dispara el correo de confirmación) — reutilizable para la migración de Odoo y cualquier alta manual futura. Correo transaccional saliendo por SMTP propio (`notificaciones@blufil.com`, Hostinger) en vez del servicio limitado de Supabase. Verificado de punta a punta en producción (aprovisionar → bloqueo hasta confirmar → login real → cambio de contraseña), datos de prueba eliminados. Falta: panel de técnicos (fuera de alcance de este primer pase) y una vía de alta real de clientes (hoy `provisionar-cliente` se invoca a mano; se conectará a la migración de Odoo o a un panel admin más adelante).
